...
class F extends CopyToTextureUtils {
  initCanvasContent({ canvasType, contextName, width, height }) {
    let canvas = null;
    ...
    const canvasContext = canvas.getContext(contextName);

    if (canvasContext === null) {
      this.skip(canvasType + ' canvas context not available');
    }

    const contextType = contextName === '2d' ? '2d' : 'gl';
  ...
  getExpectedPixels({ context, width, height, format, contextType }) {
...
g.test('copy_contents_from_canvas')
  ...
  .params(u =>
    u
      ...
      .combine('contextName', ['2d', 'webgl', 'webgl2'])
